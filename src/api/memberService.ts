...
export class MemberService {
  ...
  static async getAllMembers(): Promise<Member[]> {
    try {
      ...
      if (!response.data.success) {
        throw new Error(response.data.message || '멤버 목록 조회에 실패했습니다.');
      }
  ...
  static async getMemberById(id: number): Promise<Member> {
    try {
      const response = await api.get<MemberSingleResponse>(`${this.BASE_PATH}/${id}`);
  ...
  static async createMember(memberData: CreateMemberDto): Promise<Member> {
    try {
      ...
      const response = await api.post<MemberSingleResponse>(this.BASE_PATH, memberData);
   ...
   * @returns Promise<Member>
   */
  static async updateMember(id: number, memberData: UpdateMemberDto): Promise<Member> {
    try {
      // 입력 데이터 검증
      this.validateUpdateMemberData(memberData);
      
      const response = await api.put<MemberSingleResponse>(`${this.BASE_PATH}/${id}`, memberData);
  ...
  static async deleteMember(id: number): Promise<boolean> {
    try {
      const response = await api.delete<MemberDeleteResponse>(`${this.BASE_PATH}/${id}`);
